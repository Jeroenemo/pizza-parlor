...
Pizza.prototype.calculatePrice = function() {
  switch(true) {
    ...
    case (this.size === "Large"):
      this.price += 8;
      break;
    case (this.sauce === "White"):
      this.price += 1;
      break;
    case (this.sauce === "Secret"):
      this.price += 2;
      break;
  }
...
function displayOrder(ordertoDisplay) {
  ...
  Object.keys(order.pizzas).forEach(function(key) {
    ...
    if (order.toppings.length === 0) {
      ...
      htmlForOrder += "<li>One " + order.size + " pizza with " + order.sauce + " Sauce, " +order.toppings.join(", ").replace(/,(?=[^,]*$)/, ' and') + "</li>"
    };
  });
...
$(document).ready(function() {
  ...
  $(":button").click(function() {
    ...
    $("#shopping-cart").text("Number of items in your cart: " + pizza.id);
